...
.then(data => {
    ...
    // for 5 Dasy
    for(j = 10; j<17; j++){
        document.getElementById("day" + (j)).src = "http://openweathermap.org/img/wn/"+
        data.list[j].weather[0].icon
        +".png";
    }
    for(j = 10; j<17; j++){
        document.getElementById("mintemp" + j).innerText = "Min: " + Number(data.list[j].main.temp_min - 273.15).toFixed(2)+ "°";
    }
    
    for(j = 10; j<17; j++){
        document.getElementById("maxtemp" + j).innerText = "Max: " + Number(data.list[j].main.temp_max - 273.15).toFixed(2) + "°";
    }
})
...
function DefaultScreen(){
    document.getElementById("cityInput").defaultValue = "Ramtek";
    getInfo();
}
